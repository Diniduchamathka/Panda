...
async function connectToWA() {
  ...
  panda.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const sender = mek.key.fromMe ? panda.user.id : (mek.key.participant || mek.key.remoteJid);
    const senderNumber = sender.split('@')[0];
    const isGroup = from.endsWith('@g.us');
    const botNumber = panda.user.id.split(':')[0];
    ...
    const reply = (text) => panda.sendMessage(from, { text }, { quoted: mek });

    if (isCmd) {
      const cmd = commands.find((c) => c.pattern === commandName || (c.alias && c.alias.includes(commandName)));
      if (cmd) {
        if (cmd.react) panda.sendMessage(from, { react: { text: cmd.react, key: mek.key } });
        try {
          cmd.function(panda, mek, m, {
    ...
    for (const handler of replyHandlers) {
      if (handler.filter(replyText, { sender, message: mek })) {
        try {
          await handler.function(panda, mek, m, {
